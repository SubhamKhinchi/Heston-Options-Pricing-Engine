...
Live equity option chains → European-equivalent vol surface → Heston calibration → Model diagnostic.

## IV Surface

![Market vs Heston implied-volatility surface](assets/surface.png)

Market vs calibrated Heston vol surfaces - the model reproduces the overall level and the broad skew; the gap is concentrated in the steep short-dated put wing.
...
## 25-delta risk reversal by maturity
...
Live chain (`yfinance`) → implied forward `F(T)` per expiry from put-call parity → SOFR/OIS discount curve → de-Americanize via a CRR tree → calibrate `(v0, κ, θ, σ, ρ)` → price → surfaces, Greeks

Calibration is **Levenberg-Marquardt with a closed-form analytic Jacobian** ([Cui et al., 2016](https://doi.org/10.1016/j.ejor.2017.05.018)) on vega-weighted price residuals, over a **64-node Gauss-Legendre** characteristic-function pricer. American quotes are de-Americanized to remove the early-exercise premium and make the options chain European-equivalent.

Typical NVDA calibration run: ~360 contracts (after selecting calibration universe), converges in **~1.0s s** with **~1.2 vol pts** in-sample MAE, fitting the full liquid surface to **~1.4 vol pts** mean IV error.
...
streamlit run app/Home.py    # fetch → filter → calibrate → price → surfaces
```
